...
if __name__ == '__main__':
    ...
    new_jobs = new_df.join(old_db, on = ['job_id', 'posting_type'], how = 'anti')

    data_jobs = new_jobs.with_columns(pl.col("job_description").str.to_lowercase(), pl.col("salary_range_from").cast(pl.Float64)).filter(pl.col("job_description").str.contains('data'), pl.col("salary_range_from") >= 90000).select(['agency', 'business_title', 'civil_service_title', 'salary_range_from', 'salary_range_to', 'job_description']).sort(['agency', 'salary_range_from'])
